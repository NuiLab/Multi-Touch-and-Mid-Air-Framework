...
GLuint TAM::Helpers::LoadShaders(const char * vertex_file_path, const char * fragment_file_path){
	...
	if (VertexShaderStream.is_open()){
		...
		std::cout << "Impossible to open files\n";
		getchar();
		return 0;
	}
...
TAM::Circle::Circle(int width, int height, float rad, float r, float g, float b, float t, bool fill) {
	radius = rad;
	this->r = r;
	this->g = g;
	this->b = b;
...
void TAM::Circle::draw(InputMapper In) {
	...
	GLuint programID = newHelp.LoadShaders("/TransformVertexShader.vertexshader", "/ColorFragmentShader.fragmentshader");
	
	// Get a handle for our "MVP" uniform
	GLuint MatrixID = glGetUniformLocation(programID, "MVP");
	float ratio = (float)width / (float)height;
	float ratio_inv = (float)height / (float)width;
	...
	GLfloat g_vertex_buffer_data[3 * VERTEX_COUNT];
	int id = 0;

	for (int i = 0; i < VERTEX_COUNT; i++)
	{
		g_vertex_buffer_data[id++] = -In.getXCoordinate() + (radius * cos(((float)i * twicePi) / (float)VERTEX_COUNT));
		g_vertex_buffer_data[id++] = In.getYCoordinate() + (radius * sin(((float)i * twicePi) / (float)VERTEX_COUNT));
		g_vertex_buffer_data[id++] = centerZ;
	}

	GLfloat g_color_buffer_data[3 * VERTEX_COUNT];
	id = 0;

	for (int i = 0; i < VERTEX_COUNT; i++)
	{
		g_color_buffer_data[id++] = r;
		g_color_buffer_data[id++] = g;
		g_color_buffer_data[id++] = b;
	}
...
TAM::Square::Square(float r, float g, float b, float t, float size) {
	this->r = r;
	this->g = g;
	this->b = b;
	this->t = t;
...
void TAM::Square::draw(InputMapper In) {
	...
	GLfloat buffer[] = {
		/* First triangle */
		In.getXCoordinate(), In.getYCoordinate(), centerZ,
		In.getXCoordinate() + size, In.getYCoordinate(), centerZ,
		In.getXCoordinate(), In.getYCoordinate() + size, centerZ,

		/* Second face */
		In.getXCoordinate() + size, In.getYCoordinate() + size, centerZ,
		In.getXCoordinate() + size, In.getYCoordinate(), centerZ,
		In.getXCoordinate(), In.getYCoordinate() + size, centerZ,
	};
...
TAM::Cube::Cube(float size, ColorStruct *color)
{
	this->size = size;
...
void TAM::Cube::draw(InputMapper In)
{
	...
	if (color != NULL)
	{
		...
		{
			if (!(i % 9) && i != 0) z++;
			g_color_buffer_data[i++] = r[z];
			g_color_buffer_data[i++] = g[z];
			g_color_buffer_data[i++] = b[z];
		}
	}
...
TAM::Pyramid::Pyramid(float size, ColorStruct *color)
{
	this->size = size;
	forw = back = top = bot = left = right = 0;
	this->color = color;
