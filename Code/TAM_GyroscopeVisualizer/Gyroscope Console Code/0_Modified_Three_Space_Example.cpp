...
int main()
{
	...
	printf("==================================================\n");
	printf("Getting the Euler angles from last known orientation.\n");

	float angle[3];

	tss_error = tss_getTaredOrientationAsEulerAngles(device, angle, &timestamp);
	if (!tss_error)
	{
		printf("Euler Angles: %f, %f, %f Timestamp=%u\n", angle[0], angle[1], angle[2], timestamp);
	}
	else
	{
		printf("TSS_Error: %s\n", TSS_Error_String[tss_error]);
